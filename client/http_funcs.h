...
/*
 * Builds an HTTP request into a string based on the parameters.
 * Payload of the request is not included
 * 
 * _request - pointer to a http_request structure filled with the nessesary
 * 		parameters for the request.
 * _dest - pointer to where the request string will be stored. It should have
 * 		sufficient space.
 * _request_size - pointer to a uint32_t where the length of the string will be
 * 		stored if not NULL.
 */
 ...
 * _headers_length - pointer to a uint32_t where the length of the headers will
 ...
 * multipart/form-data upload is used.
 * 
 * _boundary - pointer to a string containing the boundary used for separation.
 * _trailer - pointer where the trailer string will be stored. It should have
 * 		sufficient space.
 * _trailer_size - pointer to a uint32_t where the length of the trailer string
 * 		will be stored if not NULL.
 */
