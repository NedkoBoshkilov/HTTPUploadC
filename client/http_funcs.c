...
static const char content_header_part1[] =	"Content-Disposition: form-data; name=\"";
static const char content_header_part2[] =	"\"; filename=\"";
static const char content_header_part3[] =	"Content-Type: application/octet-stream";
...
int
build_request(const struct http_request *request, char *dest, uint32_t *request_size)
{
	/*
	...
	for (idx_header = 0; idx_header < request->header_count; ++idx_header)
	{
		...
		if ((NULL == current_header.name) || (NULL == current_header.value))
		{
			return -1;
		}
...
int
decode_response(const char *response, struct http_response *dest, uint32_t *headers_length)
{
	bool valid;
	...
	valid = false;
	for (response_index = 0; response[response_index] != '\0'; ++response_index)
	{
		if (response_index < 3)
		{
			continue;
	...
	if (0 != dest->header_count)
	{
		...
		for (idx = 0; idx < dest->header_count; ++idx)
		{
			if (NULL == dest->headers)
			{
				...
				if ((NULL == dest->headers[idx].name) || (NULL == dest->headers[idx].value))
				{
					return -1;
				}
			}
		...
		line_lenght = next_line_start(response, response_index) - response_index - 2;
		while (line_lenght > 0)
		{
			/*
			...
			for (idx = 0; idx < dest->header_count; ++idx)
			{
				delimiter_index = next_delimiter(response, response_index);

				if (0 == strncmp(dest->headers[idx].name, &(response[response_index]), delimiter_index - response_index - 1))
				{
					...
					delimiter_index = next_line_start(response, response_index) - 2;
					strncpy(dest->headers[idx].value, &(response[response_index]), delimiter_index - response_index);
					dest->headers[idx].value[delimiter_index - response_index + 1] = '\0';
					break;
				}
			}
			response_index = next_line_start(response, response_index);
...
int
build_upload_payload_header(const char *filename, const char *form_name, const char *boundary, char *header, uint32_t *header_size)
{
	/*
...
int
build_upload_payload_trailer(const char *boundary, char *trailer, uint32_t *trailer_size)
{
	/*
